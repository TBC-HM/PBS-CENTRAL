...
export default async function CompanyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: contacts }, { data: aliases }, { data: documents }] = await Promise.all([
    ...
    supabase.from("kos_documents").select("id,title,document_type,status,updated_at").eq("workspace_id", company.workspace_id).or(`title.ilike.%${company.name.replaceAll(",", "")}%,metadata->>company_id.eq.${company.id}`).order("updated_at", { ascending: false }).limit(20)
  ]);
